...
export class GameplayRenderer {
  ...
  private getBoardAtMoveIndex(state: RootState): Map<string, PlacedTile> {
    ...
    const targetIndex = Math.min(state.ui.moveListIndex, state.game.moveHistory.length);
  ...
  private renderLastPlacedTileHighlight(state: RootState): void {
    ...
    if (state.ui.moveListIndex !== -1 && state.ui.moveListIndex !== state.game.moveHistory.length) {
      return;
    }
    ...
    connections.forEach(([dir1, dir2]) => {
      ...
      if (tileFlowEdges) {
        ...
        if (playerId) {
          ...
          return; // Unidirectional handled
        }
      }

      // Fallback: Draw flow when animation is complete but not yet in flowEdges
      if (animData && animData.animationProgress >= 1.0) {
  ...
  private renderMoveListDialog(corner: number, state: RootState): void {
    ...
    if (moveNotations.length === 0) {
      ...
      startIndex = Math.max(0, Math.min(startIndex, moveNotations.length - maxLines));
      const hasMore = startIndex > 0 || (startIndex + maxLines) < moveNotations.length;
      
      moveNotations.slice(startIndex, startIndex + maxLines).forEach((notation: string, index: number) => {
        ...
        if (isSelected) {
          ...
          this.ctx.fillRect(contentX - 5, y + 2, dialogWidth - 40, lineHeight - 4);
        }
        ...
        this.ctx.fillText(`${moveNumber}.`, contentX + numberColumnWidth, textY);
        ...
        if (move) {
          ...
          this.renderSmallTile(move.tile, previewX + previewSize/2, previewY, previewSize);
        }
      });
      ...
      if (hasMore) {
        ...
        this.ctx.fillText(`Showing ${displayedMoves} of ${moveNotations.length} moves (centered on current)`, dialogX + dialogWidth / 2, paginationY);
      }
    }
    ...
    const buttons = [
      ...
      { label: '<', x: buttonsX + buttonWidth + buttonSpacing, disabled: atFirst },
      { label: '>', x: buttonsX + 2 * (buttonWidth + buttonSpacing), disabled: atLast },
      { label: '>>', x: buttonsX + 3 * (buttonWidth + buttonSpacing), disabled: atLast },
    ];
    ...
    buttons.forEach(button => {
      ...
      this.ctx.textAlign = "center";
      this.ctx.textBaseline = "middle";
      this.ctx.fillText(button.label, button.x + buttonWidth / 2, buttonY + buttonHeight / 2);
    });
    ...
    this.ctx.fillText("Click move or use buttons to navigate", dialogX + dialogWidth / 2, instructY);
